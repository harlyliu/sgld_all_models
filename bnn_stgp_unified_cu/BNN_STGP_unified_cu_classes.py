...
class multimodal(nn.Module):
    def __init__(self, input_dim, n_hid_list, output_dim, w_dim, n_knots,
                 phi_list, eigenvalues_list, nu_tilde_list=None, act='relu'):
        super(multimodal, self).__init__()

        self.num_modalities = len(phi_list)
        if nu_tilde_list is None:
            nu_tilde_list = [0.1 for _ in range(self.num_modalities)]

        self.nn_list = nn.ModuleList([
            BNNSTGP_one_layer(
                input_dim=input_dim,
                n_hid_list=n_hid_list,
                output_dim=output_dim,
                w_dim=w_dim,
                n_knots=n_knots,
                phi=phi_list[m],
                eigenvalues=eigenvalues_list[m],
                nu_tilde=nu_tilde_list[m],
                act=act
            )
    ...
    def forward(self, x_list, w, mode):
        return sum(self.nn_list[m](x_list[m], w, mode) for m in range(self.num_modalities))

    def log_prior(self):
        return sum(nn_i.log_prior() for nn_i in self.nn_list)
    ...
    def sample_and_set_sigma_lambda_squared(self, a_lambda=2.0, b_lambda=1.0):
        for nn_i in self.nn_list:
            nn_i.sample_and_set_sigma_lambda_squared(a_lambda=a_lambda, b_lambda=b_lambda)

    def sample_and_set_sigma_theta_squared(self, a_theta=2.0, b_theta=1.0):
        for nn_i in self.nn_list:
            nn_i.sample_and_set_sigma_theta_squared(a_theta=a_theta, b_theta=b_theta)

class BNNSTGP_one_layer(nn.Module):
    def __init__(self, input_dim, n_hid_list, output_dim, w_dim, n_knots, phi, eigenvalues, nu_tilde=0.1, act='relu'):
        ...
        self.register_buffer("phi", phi if torch.is_tensor(phi) else torch.tensor(phi, dtype=torch.float32))
        self.register_buffer("eigenvalues", torch.as_tensor(eigenvalues, dtype=torch.float32, device=self.phi.device))

        values = torch.randn(n_hid_list[0], n_knots, device=self.eigenvalues.device) * self.eigenvalues
    ...
    def _setup_fc_layers(self):
        ...
        for i, output_size in enumerate(self.fc_arch[1:]):
            ...
            self._dense_weights.append(weight)
            idx += weight_size
            
            bias = self.fully_connected_nn[idx:idx + output_size]
            self._dense_biases.append(bias)
            idx += output_size
    ...
    def forward(self, x, w, mode = "train"):
        ...
        self._setup_fc_layers()
        beta = torch.mm(self.phi, self.cu)
        
        # Use the differentiable threshold during training and the hard threshold at inference.
        if mode == "train":
            beta = self.soft_threshold_approximation(beta, self.nu_tilde)
        elif mode == "inference":
            beta = self.soft_threshold(beta, self.nu_tilde)
        else:
            raise ValueError('mode can only be one of train or inference')
        
        beta = torch.sqrt(self.sigma_lambda_squared) * beta

        out = torch.mm(x, beta) + self.ksi  # (batch, U1)
        out = self.act(out)
        
        for i, (weight, bias) in enumerate(zip(self._dense_weights, self._dense_biases)):
            #multiply each layer and add bias one by one
            out = torch.mm(out, weight) + bias
            if i < len(self._dense_weights) - 1: 
                out = self.act(out)
        return out
    ...
    def sample_and_set_sigma_lambda_squared(self, a_lambda=2.0, b_lambda=1.0):
        with torch.no_grad():
            new_a = a_lambda + self.TOTAL_ENTRIES_OF_CU / 2.0
            new_b = b_lambda + 0.5 * torch.sum(self.cu ** 2)
            val = sample_inverse_gamma(
                new_a, new_b, size=1, device=self.sigma_lambda_squared.device
            ).squeeze()
            self.sigma_lambda_squared.copy_(val)
            return self.sigma_lambda_squared

    def sample_and_set_sigma_theta_squared(self, a_theta=2.0, b_theta=1.0):
        """
        Sample sigma_theta_squared from an Inverse-Gamma distribution.
        """
        with torch.no_grad():
    ...
    def soft_threshold_approximation(self, x, nu_tilde, steep=100):
        return (
            (x - nu_tilde) * self.step_approximation(x - nu_tilde, steep)
            + (x + nu_tilde) * self.step_approximation(-x - nu_tilde, steep)
        )
...
class Net(object):
    ...
    def __init__(self, task='binary', lr=1e-3, input_dim=784, n_hid_list=[32,32], output_dim=1, w_dim=1, n_knots=66,
                 N_train=200, phi_list=None, eigenvalues_list=None, nu_tilde_list=None,
        ...
        self.nu_tilde_list = nu_tilde_list
        self.num_modalities = len(self.phi_list)
        self.act = act
    ...
    def create_net(self):
        ...
        self.model = multimodal(
            ...
            phi_list=[torch.tensor(phi, dtype=torch.float32) for phi in self.phi_list],
            eigenvalues_list=self.eigenvalues_list,
            nu_tilde_list=self.nu_tilde_list,
            act=self.act
        )
...
class MNIST_runner:
    def __init__(self, train_loader, val_loader, lr=5e-7, a_list=[0.0001, 0.0001],
                 b_list=[200, 200], langevin=True, seed=17, act='relu',
                 in_feature_list=[100, 200], hidden_unit_list=[32,32],
                 step_gamma=0.5, step_decay_epoch=50, poly_degree=30,
                 nu_tilde_list=None):
        ...
        if len(self.a_list) != self.num_modalities or len(self.b_list) != self.num_modalities:
            raise ValueError("a_list and b_list must have one value per modality.")
        if nu_tilde_list is None:
            nu_tilde_list = [0.1 for _ in range(self.num_modalities)]
        if len(nu_tilde_list) != self.num_modalities:
            raise ValueError("nu_tilde_list must have one value per modality.")
        ...
        for m, in_feature in enumerate(self.in_feature_list):
            ...
            eigenvalues_np = gp_eigen_value(
                poly_degree=poly_degree,
                a=a_list[m],
                b=b_list[m]
            )
            eigenvalues_list.append(eigenvalues_np)
    ...
    def _get_saved_sample_start_idx(self, burnin_ratio=0.9):
    ...
    def get_saved_posterior_summary(self, burnin_ratio=0.9):
    ...
    def calculate_r2_from_mean_parameters(self, burnin_ratio=0.9):
    ...
    def calculate_r2_from_saved_samples(self, burnin_ratio=0.9):
        posterior_summary = self.get_saved_posterior_summary(burnin_ratio)
        weight_samples = posterior_summary["weight_samples"]
        current_state_dict = copy.deepcopy(self.net.model.state_dict())
        ...
        try:
            with torch.no_grad():
                for weight_dict in weight_samples:
                    ...
                    r2_train.append(self._calculate_r2_from_current_model(self.train_loader, mode="train"))
                    r2_test.append(self._calculate_r2_from_current_model(self.val_loader, mode="inference"))
        finally:
            self.net.model.load_state_dict(current_state_dict)

        return {
            "r2_train": np.asarray(r2_train, dtype=float),
            "r2_test": np.asarray(r2_test, dtype=float),
        }

    def calculate_beta(self, burnin_ratio=0.9):
        beta_samples_list = [[] for _ in range(self.num_modalities)]
        posterior_summary = self.get_saved_posterior_summary(burnin_ratio)
        ...
        for weight_dict in posterior_summary["weight_samples"]:
            with torch.no_grad():
                ...
                for m, nn_m in enumerate(self.net.model.nn_list):
                    beta_m = torch.mm(nn_m.phi, nn_m.cu)
                    beta_m = nn_m.soft_threshold(beta_m, nn_m.nu_tilde)
                    beta_m = torch.sqrt(nn_m.sigma_lambda_squared) * beta_m
                    beta_samples_list[m].append(beta_m.detach().cpu().numpy())

        beta_arr_list = [np.stack(beta_samples, axis=0) for beta_samples in beta_samples_list]
        return beta_arr_list

    def calculate_mask(self, beta, gamma=0.5):
        if beta.ndim == 3:
            p_hat_unit_voxel = (beta != 0).mean(axis=0).T
            p_hat = p_hat_unit_voxel.mean(axis=0)
        else:
            p_hat = (beta != 0).mean(axis=0)
    ...
    def compute_selected_voxel_proportions(self, c=1.0):
        ...
        with torch.no_grad():
            for m, nn_m in enumerate(self.net.model.nn_list):
                beta_r_m = torch.mm(nn_m.phi, nn_m.cu)
                beta_r_m = nn_m.soft_threshold(beta_r_m, nn_m.nu_tilde)
                beta_r_m = torch.sqrt(nn_m.sigma_lambda_squared) * beta_r_m
...
class Experiment:
    ...
    def run_whole_experiment(self, sample_size, data_type, seed, true_r2, gamma,
                             in_feature_list, a_list, b_list, hidden_unit_list,
                             step_gamma, step_decay_epoch, lr, epochs, poly_degree,
                             val_ratio=0.5, nu_tilde_list=None):
        self.set_seed(seed)
    ...
    def process_results(self, trainer, burnin_ratio=0.9, gamma=0.1, true_beta_list=None, sample_size=None):
        ...
        saved_sample_r2 = trainer.calculate_r2_from_saved_samples(burnin_ratio)
        mean_parameter_r2 = trainer.calculate_r2_from_mean_parameters(burnin_ratio)
        print(f"Sample Size: {sample_size}")
        print(f"Average saved r2_train:{np.nanmean(saved_sample_r2['r2_train'])}")
        print(f"Average saved r2_test:{np.nanmean(saved_sample_r2['r2_test'])}")
        ...
        run_result = {
            ...
            "saved_r2_train": saved_sample_r2['r2_train'],
            "saved_r2_test": saved_sample_r2['r2_test'],
            "logged_saved_r2_train": posterior_summary['r2_train'],
            "logged_saved_r2_test": posterior_summary['r2_test'],
