...
from BNN_STGP_unified_cu_classes import Experiment
...
def result_path(output_dir, true_r2, seed, epochs, lr, poly_degree):
    return (
        output_dir
        / (
            f"results_r2_{format_float_for_path(true_r2)}_epochs_{epochs}_"
...
def run_label(row):
    return (
        f"r2_{format_float_for_path(row['true_r2'])}_epochs_{row['epochs']}_"
...
def run_one(seed, true_r2, epochs, burnin_ratios, output_dir):
    print(f"Starting true_r2={true_r2}, seed={seed}, epochs={epochs}")
    ...
    trainer = experiment.run_whole_experiment(
        ...
        a_list=RUN_CONFIG["a_list"],
        b_list=RUN_CONFIG["b_list"],
    ...
    for burnin_ratio in burnin_ratios:
        print(f"Processing true_r2={true_r2}, seed={seed}, burnin_ratio={burnin_ratio}")
...
def result_files(output_dir):
    ...
    files = []
    for path in sorted(output_dir.glob("results_r2_*_epochs_*_seed_*_lr_*_polydegree_*.npz")):
...
def export_results(output_dir):
    ...
    groups = {}
    for row in files:
        key = (row["true_r2"], row["epochs"], row["lr"], row["poly_degree"])
