...
class multimodal(nn.Module):
    def __init__(self, input_dim, n_hid_list, output_dim, w_dim, n_knots,
        ...
        self.nn_list = nn.ModuleList([
            BNNSTGP_one_layer(
                ...
                w_dim=w_dim,
                n_knots=n_knots,
                phi_cr=phi_cr_list[m],
                phi_cw=phi_cw_list[m],
                lambda_cr=lambda_cr_list[m],
                lambda_cw=lambda_cw_list[m],
                act=act
            )
    ...
    def sample_and_set_sigma_lambda_r_squared(self, a_lambda_r=2.0, b_lambda_r=1.0):
        for nn_i in self.nn_list:
            nn_i.sample_and_set_sigma_lambda_r_squared(a_lambda_r=a_lambda_r, b_lambda_r=b_lambda_r)

    def sample_and_set_sigma_lambda_w_squared(self, a_lambda_w=2.0, b_lambda_w=1.0):
        for nn_i in self.nn_list:
            nn_i.sample_and_set_sigma_lambda_w_squared(a_lambda_w=a_lambda_w, b_lambda_w=b_lambda_w)
...
class BNNSTGP_one_layer(nn.Module):
    def __init__(self, input_dim, n_hid_list, output_dim, w_dim, n_knots, phi_cr, phi_cw, lambda_cr, lambda_cw, act='relu'):
        ...
        self.register_buffer("phi_cr", phi_cr if torch.is_tensor(phi_cr) else torch.tensor(phi_cr, dtype=torch.float32))
        self.register_buffer("phi_cw", phi_cw if torch.is_tensor(phi_cw) else torch.tensor(phi_cw, dtype=torch.float32))
        self.register_buffer("lambda_cr", torch.as_tensor(lambda_cr, dtype=torch.float32, device=self.phi_cr.device))
        self.register_buffer("lambda_cw", torch.as_tensor(lambda_cw, dtype=torch.float32, device=self.phi_cw.device))
        
        # --- NEW: two sigma_lambda^2 scalars
        self.sigma_lambda_r_squared = nn.Parameter(torch.tensor(1.0), requires_grad=False)
        self.sigma_lambda_w_squared = nn.Parameter(torch.tensor(1.0), requires_grad=False)
        
       # --- New: cr generation
        values_r = torch.randn(n_knots, device=self.lambda_cr.device) * self.lambda_cr
        self.c_r = nn.Parameter(values_r)  # shape: (K,)
        self.TOTAL_ENTRIES_OF_CR = self.c_r.numel()
        
        # --- NEW: weighting coefficients (K, U1) like old cu
        values_w = torch.randn(n_knots, n_hid_list[0], device=self.lambda_cw.device) * self.lambda_cw.unsqueeze(1)
        self.c_w = nn.Parameter(values_w)  # shape: (K, U1)
        self.TOTAL_ENTRIES_OF_CW = self.c_w.numel()
    ...
    def forward(self, x, w, mode = "train"):
        ...
        beta_r = torch.mv(self.phi_cr, self.c_r)  # (V,)
        beta_r = torch.sqrt(self.sigma_lambda_r_squared) * beta_r
        
        # indicator approximation during training, hard indicator at inference
        if mode == "train":
            # I(x > 0) ≈ 0.5 * (1 + tanh(k*x))
        ...
        beta_w = torch.mm(self.phi_cw, self.c_w)  # (V, U1)
        beta_w = torch.sqrt(self.sigma_lambda_w_squared) * beta_w
        
        beta_eff = beta_w * mask.unsqueeze(1)  # (V, U1)
        
        out = torch.mm(x, beta_eff) + self.ksi  # (batch, U1)
        out = self.act(out)
    ...
    def log_prior(self):
        logprior = 0.5 * ((self.c_r ** 2) / self.lambda_cr).sum() / self.sigma_lambda_r_squared
        logprior += 0.5 * ((self.c_w ** 2) / self.lambda_cw.unsqueeze(1)).sum() / self.sigma_lambda_w_squared
    ...
    def sample_and_set_sigma_lambda_r_squared(self, a_lambda_r=2.0, b_lambda_r=1.0):
        with torch.no_grad():
            new_a = a_lambda_r + self.TOTAL_ENTRIES_OF_CR / 2.0
            new_b = b_lambda_r + 0.5 * ((self.c_r ** 2) / self.lambda_cr).sum()
            val = sample_inverse_gamma(
                new_a, new_b, size=1, device=self.sigma_lambda_r_squared.device
            ).squeeze()
            # print(f"sigma_lambda_r_squared{val}")
            self.sigma_lambda_r_squared.copy_(val)
            return self.sigma_lambda_r_squared
    
    def sample_and_set_sigma_lambda_w_squared(self, a_lambda_w=2.0, b_lambda_w=1.0):
        with torch.no_grad():
            new_a = a_lambda_w + self.TOTAL_ENTRIES_OF_CW / 2.0
            new_b = b_lambda_w + 0.5 * ((self.c_w ** 2) / self.lambda_cw.unsqueeze(1)).sum()
            val = sample_inverse_gamma(
                new_a, new_b, size=1, device=self.sigma_lambda_w_squared.device
            ).squeeze()
            self.sigma_lambda_w_squared.copy_(val)
            return self.sigma_lambda_w_squared
...
class MNIST_runner:
    def __init__(self, train_loader, val_loader, lr=5e-7, a_for_eigen_cr_list=[0.0001, 0.0001],
                 ...
                 b_for_eigen_cw_list=[200, 200], langevin=True, 
        ...
        for weight_dict in posterior_summary["weight_samples"]:
            with torch.no_grad():
                ...
                for m, nn_m in enumerate(self.net.model.nn_list):
                    beta_r_m = torch.mv(nn_m.phi_cr, nn_m.c_r)
                    beta_r_m = torch.sqrt(nn_m.sigma_lambda_r_squared) * beta_r_m
...
class Experiment:
    ...
    def run_whole_experiment(self, sample_size, data_type='npz', seed=42, true_r2=0.8,
                             ...
                             b_for_eigen_cr_list=[200, 200], a_for_eigen_cw_list=[0.0001, 0.0001],
                             b_for_eigen_cw_list=[200, 200], in_feature_list=[100, 200],
