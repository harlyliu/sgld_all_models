...
from Cnn_classes_4_9 import Experiment
...
def result_path(output_dir, true_r2, seed, epochs, lr):
    return (
        output_dir
        / f"cnn_results_r2_{format_float_for_path(true_r2)}_epochs_{epochs}_seed_{seed}_lr_{format_float_for_path(lr)}.npz"
    )


def run_label(true_r2, epochs, lr):
    return f"cnn_r2_{format_float_for_path(true_r2)}_epochs_{epochs}_lr_{format_float_for_path(lr)}"
...
def run_one(seed, true_r2, epochs, output_dir):
    print(f"Starting CNN true_r2={true_r2}, seed={seed}, epochs={epochs}")
...
def result_files(output_dir):
    ...
    rows = []
    for path in sorted(output_dir.glob("cnn_results_r2_*_epochs_*_seed_*_lr_*.npz")):
        try:
...
def write_summary_csv(files, output_dir):
    output_path = output_dir / "cnn_results_summary.csv"
...
def plot_r2_grid(group_rows, output_dir, which, ncols=5):
    ...
    first = group_rows[0]
    fig.suptitle(f"{run_label(first['true_r2'], first['epochs'], first['lr'])}, {which} R2", fontsize=12)
    save_figure(fig, output_dir, f"{run_label(first['true_r2'], first['epochs'], first['lr'])}_{which}_r2_grid.png")


def export_results(output_dir):
    files = result_files(output_dir)
    if not files:
        print(f"No CNN result files found in {output_dir}; skipping CSV/plots.")
        return
...
def main():
    output_dir = Path(os.environ.get("CNN_OUTPUT_DIR", DEFAULT_OUTPUT_DIR)).expanduser().resolve()
    epochs = int(os.environ.get("CNN_EPOCHS", DEFAULT_EPOCHS))
