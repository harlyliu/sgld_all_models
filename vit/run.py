...
from ViT_classes_4_4_2026 import Experiment
...
def result_path(output_dir, true_r2, seed, epochs, lr):
    return (
        output_dir
        / f"vit_results_r2_{format_float_for_path(true_r2)}_epochs_{epochs}_seed_{seed}_lr_{format_float_for_path(lr)}.npz"
    )


def run_label(true_r2, epochs, lr):
    return f"vit_r2_{format_float_for_path(true_r2)}_epochs_{epochs}_lr_{format_float_for_path(lr)}"
...
def run_one(seed, true_r2, epochs, output_dir):
    print(f"Starting ViT true_r2={true_r2}, seed={seed}, epochs={epochs}", flush=True)
    ...
    with temp_path.open("wb") as handle:
        np.savez_compressed(
            ...
            data_type=np.array(RUN_CONFIG["data_type"]),
...
def result_files(output_dir):
    ...
    rows = []
    for path in sorted(output_dir.glob("vit_results_r2_*_epochs_*_seed_*_lr_*.npz")):
        try:
            ...
            print(f"Skipping unreadable result file {path}: {exc}", flush=True)
...
def write_summary_csv(files, output_dir):
    output_path = output_dir / "vit_results_summary.csv"
...
def export_results(output_dir):
    ...
    if not files:
        print(f"No ViT result files found in {output_dir}; skipping CSV/plots.", flush=True)
        return
...
def main():
    output_dir = Path(os.environ.get("VIT_OUTPUT_DIR", DEFAULT_OUTPUT_DIR)).expanduser().resolve()
    epochs = int(os.environ.get("VIT_EPOCHS", DEFAULT_EPOCHS))
    ...
    print("Completed ViT runs.", flush=True)
